...
| **Report Writer** | Scans an entire scene's UI for contrast and font-size violations, exports a JSON report | none |
| **Font Scaler** | Globally scales UI text and highlights any elements that start overlapping as a result | none |

## Requirements

- Unity 6000.0.x LTS or later (built and tested on 6000.0.81f1)
- **Universal Render Pipeline (URP)**, required only for Shader Controller (CVD simulation and blur). Contrast Meter, Report Writer, and Font Scaler work in any render pipeline, including Built-in RP.
- TextMeshPro (Unity's standard package), used for font-size and overlap detection alongside legacy `Text` support.

If your project doesn't have URP installed, or your URP Renderer Feature isn't set up, the tool detects this automatically and hides the Shader Controller section of its window rather than throwing errors. See [Non-URP projects](#non-urp-projects) below.
...
`Runtime/` has no dependency on `UnityEditor`, so none of it gets stripped from a player build. This is where the actual logic lives, one folder per module. `Editor/` is the EditorWindow itself plus a small bridge class per module that wires the Runtime logic up to buttons and fields in the tool window. `Tests/` and `TestAssets/` only matter if you clone the repo; they're left out of the exported `.unitypackage` on purpose, since there's no reason to ship test scaffolding to someone who just wants the tool.

## Quick start

1. **Window → Accessibility UI Tester** to open the tool.
2. If your project uses URP and you want CVD/blur simulation, drag your project's **Universal Renderer Data** asset into the **URP Renderer Data** field (not the URP Pipeline Asset itself; see [Finding your Renderer Data asset](#finding-your-renderer-data-asset)).
3. Everything else works immediately. No further setup is needed for Contrast Meter, Report Writer, or Font Scaler.
...
2. The first time you do this for a given Renderer Data asset, you'll need to add the feature once: select the Renderer Data asset in your Project window, open its Inspector, click **Add Renderer Feature → CVD Renderer Feature**, then drag the `CVDSimulation` and `GaussianBlur` shaders (from `Runtime/Shaders/`) into the **Cvd Shader** and **Blur Shader** fields.
3. Click **Toggle CVD Simulation** in the tool window to cycle through None, Protanopia, Deuteranopia, and Tritanopia.
4. Click **Toggle Low Vision Blur** to enable or disable the blur pass independently.
...
1. Assign an **AccessibilityThresholds** asset in the tool window. A default one, `DefaultAccessibilityThresholds`, is included, or create your own via **Assets → Create → Accessibility Tester → Thresholds** to set custom contrast and font-size minimums.
...
3. The "Scan in Play Mode" checkbox handles scenes where UI backgrounds depend on runtime camera behaviour: a parallax background, say, or a camera that only reaches its correct position once gameplay starts. Check the box, and the tool enters Play Mode, waits for the scene to settle, scans, then returns to Edit Mode automatically. One real limit here: that automated wait can only settle on whatever screen your game happens to reach on its own (often a title screen). It can't navigate menus for you. For scenes reached only after specific player input, get your game into that state manually first, then click Generate Report while genuinely in Play Mode; the tool detects this and skips the wait cycle.
4. Each element in the report includes a `backgroundConfidence` field explaining how its background colour was determined. `High` means it was read directly or rendered accurately; `Estimated` means it was approximated from a texture; `Low` means a fallback was used that may not be fully accurate, with the reason stated. Check this field before treating a borderline result as final.

### Font Scaler

1. Set the **Scale Factor** slider (0.5x to 3x).
...
- Report Writer can't determine the true background colour behind text with no ancestor UI graphic, when that background is a coloured sprite read through a white tint multiplier and the sprite's texture isn't marked "Read/Write Enabled" in its import settings. This is disclosed in the report's `backgroundConfidence` field rather than silently guessed.
- "Scan in Play Mode" can't navigate your game's UI for you. It settles at whatever screen your game reaches on its own after entering Play Mode, commonly a title or start screen. To scan gameplay-specific UI states, play to that state manually first, then click Generate Report.
- Report Writer's Edit Mode scans render the scene's camera(s) to an offscreen texture to determine backgrounds. Multi-camera scenes, such as a layered background plus gameplay plus UI camera rig, are composited in depth order automatically, though this adds a small amount of scan time compared to a purely static colour lookup.

## Non-URP projects

If URP isn't installed, or the `ACCESSIBILITY_TESTER_URP` scripting define isn't set in your project's Player Settings, the Shader Controller UI is hidden automatically and everything else works normally. No manual file deletion is needed, and no errors are thrown.

If you do have URP installed and want Shader Controller available, add `ACCESSIBILITY_TESTER_URP` under **Edit → Project Settings → Player → Other Settings → Scripting Define Symbols**.
...
2. **EditMode** tab, **Run All**. Covers WCAG contrast math and CVD matrix accuracy, verified against Machado, Oliveira & Fernandes 2009's published reference data.
